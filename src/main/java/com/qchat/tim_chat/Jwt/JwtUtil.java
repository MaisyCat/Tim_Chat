...
//                .id(uuid)
    ...
    /**
     * 生成 Refresh Token（包含设备编码）
     * 
     * @param username 用户名
     * @param deviceCode 设备编码（DeviceID + OAID 的混合编码）
     * @return JWT Refresh Token 字符串（包含设备信息）
     */
    public String genRefreshToken(String username, String deviceCode) {
        return Jwts.builder()
                .header()
                .add("typ", "JWT")
                .add("alg", "Hs256")
...
//                .id(uuid)
     ...
     * 从 JWT Token 中获取设备编码
     * @param token JWT Token 字符串
     * @return 设备编码（DeviceID + OAID）
     */
    public String getDeviceCodeFromToken(String token) {
        return Jwts.parser()
                .verifyWith(KEY)
                .build()
                .parseSignedClaims(token)
                .getPayload()
                .get("deviceCode", String.class);
    }
